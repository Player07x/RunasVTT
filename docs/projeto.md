...
**Correção (2026-09-21, `v0.3.3`):** o jogador abria o Tools mas não conseguia abrir a ficha. O espelho copiava só os arquivos escritos por extenso nas páginas; os pedaços que o Tools carrega por `next/dynamic` — a ficha, as calculadoras — têm o nome montado em tempo de execução e só aparecem na lista de pré-cache do service worker, em caminho relativo, que o descobridor de assets não reconhecia. Como o service worker do Tools é bloqueado no assento, esses arquivos nunca chegavam ao jogador e o `import()` da ficha caía em 404. `extractAssetUrls` passou a aceitar `"./\_next/static/…"`; a cópia do Tools foi de 36 para 60 arquivos e a ficha abre pelo espelho, sem rede.
...
|Data|Mudança|
|-|-|
|2026-09-22|Ficha do assento deixava de valer ao mover o token. `syncSeatCharacterFromToken` roda a cada gravação do token — e um arraste no mapa é uma gravação —, e reescrevia a ficha subindo a revisão toda vez. O aviso derrubava o que o jogador estava digitando, e o `PUT` seguinte chegava com `baseRevision` velho e voltava 409; como o jogador move o próprio token (ADR 0017), ele apagava a própria ficha ao andar. Agora a gravação só acontece quando envelope, resumo, token, cena ou tamanho mudam de fato (`sameSeatCharacter`); posição e o resto do token não pertencem à ficha. Teste de integração cobre os dois lados: mover não mexe na revisão, editar continua avisando o jogador.|
|2026-09-22|Tráfego do assento pelo túnel, com todos os jogadores remotos. Medido: com a tela parada, o shim relia a ficha **2 vezes a cada 2,5 s** (~48/min por jogador), e cada leitura devolve o `envelope` inteiro mais o `tokenImage` em base64 — ~209 KB numa ficha com imagem, ou **~10 MB/min** subindo pelo upload da casa do mestre, por jogador, sem ninguém tocar em nada. A sondagem passa a existir só quando o WebSocket está fechado e só avisa quando a revisão muda de fato; leituras simultâneas são deduplicadas e reaproveitadas por 1 s. Medido depois: **1 leitura, e para**. Em paralelo, `serveTools` respondia `no-cache` **sem validador** — o espelho descarta o ETag da origem —, então cada navegação dentro do Tools rebaixava todo o JavaScript pelo túnel; agora `/_next/static/` (nome com hash) vai como `immutable` e o resto ganha ETag com 304. Medido: ao voltar para a Galeria, 12 de 12 arquivos vieram do cache local, 0 bytes de rede. Também: botão **Limpar** no Registro, que só crescia. **Ainda não lançado:** está em `main`, sem tag — o instalador publicado segue o `v0.3.4`.|
|2026-09-21|Correção do `crypto.randomUUID` no assento: `http://IP:porta` não é contexto seguro e o navegador esconde `randomUUID` aí — só `127.0.0.1` escapa, que é justamente a janela local do mestre, por isso a falha não aparecia nos testes locais nem no site publicado (HTTPS). O Runas Tools chama `randomUUID` sem proteção ao importar, salvar e criar fichas, e o shim o chamava no `mutationId`: o jogador da rede local via "crypto.randomUUID is not a function" no ZIP (sem `try`) e "arquivo de ficha inválido" no JSON avulso (a mensagem enganosa do `catch`). O shim passa a repor `randomUUID` por `getRandomValues` (UUID v4 válido, disponível fora de contexto seguro) e é injetado no **começo** do `<head>`, antes dos scripts do Tools. Medido em `http://192.168.15.12`: importar falhava e passou a funcionar; removida a reposição em tempo de execução, a falha volta. Release `v0.3.4`.|
|2026-09-21|Correção da ficha do jogador: a cópia local nunca guardava os pedaços que o Runas Tools carrega sob demanda. Os nomes desses arquivos são montados em tempo de execução e só aparecem escritos na lista de pré-cache do service worker, em caminho relativo (`"./\_next/static/chunks/…"`), que `extractAssetUrls` ignorava — faltavam 24 dos 43 arquivos de JavaScript/CSS do build, entre eles os das seções da ficha. No assento o service worker é bloqueado de propósito (ADR 0020), então o jogador dependia só da cópia local: o `import()` batia num 404 e a ficha não abria. O descobridor passa a aceitar o caminho relativo, e a cópia do Tools vai a 60 arquivos. Release `v0.3.3`.|

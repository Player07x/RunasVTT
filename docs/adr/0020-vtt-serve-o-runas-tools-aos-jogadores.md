...
- **A origem do jogador muda a cada sessão** (`http://IP:porta` ou o endereço do túnel). Consequências aceitas:
  ...
  - **o contexto não seguro tira mais que o PWA:** `crypto.randomUUID` e `crypto.subtle` não existem em `http://IP:porta`. `127.0.0.1` é exceção, então a janela local do mestre esconde o problema e só o jogador da rede o encontra — o Tools chama `randomUUID` ao importar, salvar e criar fichas. O shim repõe `randomUUID` por `getRandomValues` e é injetado no começo do `<head>`, para rodar antes do Tools (`v0.3.4`); qualquer outra API que o Tools exija e que dependa de contexto seguro precisa do mesmo tratamento;
